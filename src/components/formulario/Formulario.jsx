    ...
    const manejadorFormulario = async (e) => {
        ...
        const orden = {
            items: carrito.map(producto => ({
                id: producto.id || 'ID no disponible',
        ...
        try {
            ...
            setEmail('');
            setEmailValido('');
            setError('');  // Limpiar errores después del éxito
        } catch (error) {
...
<div className='input_cajita'>
    <label htmlFor="input_nombre">
        <input type="text" name='nombreusuario' id='input_nombre'
...
<div className='input_cajita'>
    <label htmlFor="input_apellido">
        <input type="text" name='apellidousuario' id='input_apellido'
...
<div className='input_cajita'>
    <label htmlFor="input_telefono">
        <input type="text" name='telefonousuario' id='input_telefono'
            required placeholder='N° de Contacto'
...
<div className='input_cajita'>
    <div className={`input_email ${focus ? 'focus' : ''}`}>
        ...
        <div className="icons">
            <img className={`icon_check ${isValid === true ? 'show' : ''}`} src={iconCheck} />
            <img className={`icon_error ${isValid === false ? 'show' : ''}`} src={iconError} />
        </div>
    </div>
</div>
    ...
    <div className={`input_emailvalidar ${focus ? 'focus' : ''}`}>
        <label htmlFor="input_emailvalidar">
            <input type="email" name="emailvalidar" id="input_emailvalidar"
                required placeholder='Confirma tu Email' value={emailValido}
                onChange={(e) => setEmailValido(e.target.value)}
                onFocus={() => setFocus(true)}
                onBlur={() => setFocus(false)} />
        </label>
        <div className="icons">
            <img className={`icon_check ${isValid === true && email === emailValido ? 'show' : ''}`} src={iconCheck} />
            <img className={`icon_error ${isValid === false || email !== emailValido ? 'show' : ''}`} src={iconError} />
        </div>
    </div>
</div>
